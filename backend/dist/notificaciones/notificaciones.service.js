...
let NotificacionesService = class NotificacionesService {
    ...
    async onModuleInit() {
        await this.db
            .query(`CREATE UNIQUE INDEX IF NOT EXISTS ux_notificaciones_infraccion
    ...
    async loadInfraccion(infraccionId) {
    ...
    async generarPdf(infraccionId) {
        ...
        const up = await this.db.query(`INSERT INTO notificaciones (infraccion_id, pdf_path, estado)
       VALUES ($1, $2, 'generado')
       ON CONFLICT (infraccion_id) DO UPDATE
         SET pdf_path = EXCLUDED.pdf_path,
             estado   = CASE
                          ...
                          ELSE 'generado'
                        END
       RETURNING id, infraccion_id, pdf_path, estado, created_at, sent_at`, [infraccionId, outPath]);
        const n = up.rows[0];
        return { id: n.id, pdf_path: n.pdf_path, estado: n.estado, filename };
    }
    async generarPdfStream(infraccionId) {
        const row = await this.loadInfraccion(infraccionId);
        if (!row)
            throw new Error('Infracción no encontrada');
        const filename = `ACTA-${this.actaNro(row)}.pdf`;
    ...
    async enviar(notificacionId, email) {
        const r = await this.db.query(`SELECT n.id, n.pdf_path, n.estado, n.infraccion_id,
              (i.serie || '-' || lpad(i.nro_correlativo::text, 7, '0')) AS nro_acta
         FROM notificaciones n
         JOIN infracciones i ON i.id = n.infraccion_id
        WHERE n.id=$1`, [notificacionId]);
        const row = r.rows[0];
        if (!row)
        ...
        const upd = await this.db.query(`UPDATE notificaciones
          SET estado='enviado', email=$1, sent_at=now()
        WHERE id=$2
        RETURNING infraccion_id`, [email, notificacionId]);
        const infId = upd.rows[0]?.infraccion_id ?? row.infraccion_id;
        await this.db.query(`UPDATE infracciones
          SET notificado = TRUE,
